...
/**
 * M06 신규 - Parser가 만들어낸 정규화된 텍스트(전달 즉시 소비되는 일시적
 * 값 - M07A/V006 이후 {@code document_extracted_content.normalized_text}
 * 같은 어떤 테이블에도 영속 저장되지 않는다, v1.4 §2A.3) 안의 한 구간이
 ...
 *
 * <p>{@code startOffset}/{@code endOffset}은 그 정규화된 텍스트에 대한 Java
 * {@link String}과 동일한 단위(UTF-16 code unit index, {@code [start, end)}
 * 반개구간)다 - Python {@code parser_service.py}는 Python {@code str}
 * (코드포인트) 오프셋이 아니라 이 단위로 변환해 응답해야 한다(Non-BMP
 * 문자는 Java에서 Surrogate Pair 2개로 계산되므로 단순 코드포인트 수와
 * 다를 수 있다) - 그렇지 않으면 Java 쪽에서 저장된 오프셋으로 부분 문자열을
 * 잘랐을 때 위치가 어긋난다.</p>
 */
