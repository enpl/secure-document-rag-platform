...
## 8. 사용자 레지스트리와 clearance 초기 설정 (V013 이후)

V013 적용 뒤에는 사용자가 인증된 기본 Home에 도착하면 공통 Shell이 `GET /api/me`를 한 번 호출한다. 이 요청은 서버가 검증한 JWT의 issuer+subject와 로그인 ID만 사용해 `sdv_users` projection을 생성하거나 안전한 표시 label을 갱신한다. 질문 전송, Google 연결, Drive 방문은 등록 조건이 아니다. 브라우저가 보낸 이름이나 subject를 신원 증거로 사용하지 않는다.

최초 등록 상태는 `CLEARANCE_UNSET`이며 공유 콘텐츠 권한이 아니다. ADMIN은 **사용자 접근** 화면에서 로그인 ID로 이미 SDV에 로그인한 사용자를 찾아 필요한 최대 열람 등급과 active 상태만 명시적으로 저장한다. 로그인 반복은 clearance, active 상태, authorization revision을 초기화하거나 비활성 사용자를 다시 활성화하지 않는다. `REGISTRY_UNAVAILABLE` 또는 bootstrap 실패가 보이면 준비 완료로 간주하지 말고 화면의 **다시 확인**을 한 번 사용한다. 계속 실패하면 토큰의 필수 identity claim과 backend 상태를 content-free 방식으로 점검한다.

이 디렉터리는 검증된 SDV 로그인/bootstrap을 마친 사용자만 포함하며, 아직 한 번도 로그인하지 않은 Keycloak realm 전체를 조회하지 않는다. realm-admin credential은 필요하지 않다.
